...
//listBarang
function tampilkanData(filterText = ""){
    if (!tableBodyList) return; 
    let listBarangList = JSON.parse(localStorage.getItem('stokBarang')) || [];
    tableBodyList.innerHTML = ""; 

    listBarangList.forEach((item, index) =>{
        if (item.nama.toLowerCase().includes(filterText.toLowerCase())) {
            let hargaBeliFmt = "RP " + Number(item.beli).toLocaleString('id-ID');
            let hargaJualFmt = "RP " + Number(item.jual).toLocaleString('id-ID');
            ...
            let row = `<tr>
                ...
                <td>${item.jumlah}</td>
                <td>${item.jenis}</td>
                <td>${hargaBeliFmt}</td> <td>${item.kadaluarsa}</td>
                <td>
                </td>
            </tr>`;
            tableBodyList.innerHTML += row;
        }
    });
}
